...
columns_to_select = [dtype not in (ltype.int, ltype.real)
...
matching_frames = [frame for frame in DT 
                   if frame.ltypes[0] not in (ltype.real, ltype.int)]
...
columns_to_select = [ltype in (ltype.real, ltype.int) 
                     and DT[name].mean()[0, 0] > 10
...
- Based on datatable version ``1.0.0a0+build.1612992576``